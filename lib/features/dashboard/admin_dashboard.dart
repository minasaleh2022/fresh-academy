  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GridView.count(
        ...
        mainAxisSpacing: 12,
        children: [
          if (RBAC.canManageUsers(role!))
            GestureDetector(
              onTap: () => context.push('/departments/manage'),
              child: const StatCard(
                  title: 'الأقسام', value: '—', icon: Icons.apartment_outlined),
            ),
          if (RBAC.canManageCatalog(role!))
            GestureDetector(
          ...
          if (RBAC.canManageUsers(role!))
            GestureDetector(
              onTap: () => context.push('/users/trainees'),
              child: const StatCard(
                  title: 'المتدربون', value: '—', icon: Icons.group_outlined),
            ),
          if (RBAC.canSeeReports(role!))
            GestureDetector(
              onTap: () => context.push('/reports/dashboard'),
              child: const StatCard(
                  title: 'التقارير', value: '—', icon: Icons.bar_chart_outlined),
            ),
          GestureDetector(
          ...
          GestureDetector(
            onTap: () => context.push('/dashboard/modules'),
            child: const StatCard(
              title: 'وحدة CRUD الشاملة',
              value: 'كل القوائم',
